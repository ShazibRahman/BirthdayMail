...
import logging
logging.basicConfig(filename=os.path.dirname(__file__)+'/logger.log', filemode='a', level=logging.DEBUG,format='%(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p')
...
class BirthdayMail:
    ...
    def message_func(self,receiver_email,name):
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", 465, context= ssl.create_default_context()) as server:
            ...
            logging.info(f"The email has been sent to {name} with email {receiver_email}")
